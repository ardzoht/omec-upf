...
func (q *Qer) parseQER(ie1 *ie.IE, seid uint64) error {
	qerID, err := ie1.QERID()
	if err != nil {
		log.Info("Could not read QER ID!")
		return err
	}

	qfi, err := ie1.QFI()
	if err != nil {
		log.Info("Could not read QFI!")
	}

	gsUL, err := ie1.GateStatusUL()
	if err != nil {
		log.Info("Could not read Gate status uplink!")
	}

	gsDL, err := ie1.GateStatusDL()
	if err != nil {
		log.Info("Could not read Gate status downlink!")
	}

	mbrUL, err := ie1.MBRUL()
	if err != nil {
		log.Info("Could not read MBRUL!")
	}

	mbrDL, err := ie1.MBRDL()
	if err != nil {
		log.Info("Could not read MBRDL!")
	}

	gbrUL, err := ie1.GBRUL()
	if err != nil {
		log.Info("Could not read GBRUL!")
	}

	gbrDL, err := ie1.GBRDL()
	if err != nil {
		log.Info("Could not read GBRDL!")
	}
